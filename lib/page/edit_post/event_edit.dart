...
class EventEdit extends StatefulWidget {
  ...
  @override
  EventEditState createState() => EventEditState();
}
...
// コントローラ---------------------------------------------
  ...
  late TextEditingController detailCon;
  late TextEditingController postalCon;
  late TextEditingController postalFrCon;
  late TextEditingController postalBaCon;
  late TextEditingController phoneCon;
  late TextEditingController phoneFrCon;
  late TextEditingController phoneCenCon;
  late TextEditingController phoneBaCon;
  late TextEditingController emailCon;
  late TextEditingController homepageCon;
  late TextEditingController addCon;
  late TextEditingController cautionCon;
  late TextEditingController feeController;
  late TextEditingController capaController;
  List<String> parts = [];
  String dayTimeGet = "";
  String dayTimeGetStr = "";
  String dayTimeGetFin = "";
...
// 確認ボタンを押すときにmapをつくりなおしているので，押す前に要素を使わなければエラーは出ないが念のために初期化している
  ...
  EventEditState() {
    // titleCon.text = eventDetailList["title"];
    // detailCon = eventDetailList["detail"];
    // postalCon = eventDetailList["postalNumber"];
    // if (eventDetailList["phone"] != "") {
    //   phoneCon = eventDetailList["phone"];
    // }
    // if (eventDetailList["mail"] != "") {
    //   emailCon = eventDetailList["mail"];
    // }
    // if (eventDetailList["url"] != "") {
    //   homepageCon = eventDetailList["url"];
    // }
    // if (eventDetailList["fee"] != "無料") {
    //   feeController = eventDetailList["fee"];
    // }
    // capaController = eventDetailList["Capacity"];
    // addCon = eventDetailList["addMessage"];
    // caution = eventDetailList["notes"];
...
//編集用更新---------------------------------------------------------------------
  ...
  void initState() {
    super.initState();
    // planId = widget.eventList["purchase"]["plan_id"];
    // planPeriod = widget.eventList["purchase"]["planPeriod"];
    eventDetailList = widget.eventList;
    imageUrl = eventDetailList["image_url"];
    planId = eventDetailList["id"];

    titleCon = TextEditingController(text: eventDetailList["title"]);
    postTitle = eventDetailList["title"];

    detailCon = TextEditingController(text: eventDetailList["detail"]);
    detail = eventDetailList["detail"];

    postalCon = TextEditingController(text: eventDetailList["postalNumber"]);
    parts = eventDetailList["postalNumber"].split('-');
    postalFrCon = TextEditingController(text: parts[0]);
    postalBaCon = TextEditingController(text: parts[1]);
    firstNumber = parts[0];
    backNumber = parts[1];
    posAddressNum = eventDetailList["postalNumber"];
    posPrefecture = eventDetailList["prefecture"];
    posCity = eventDetailList["city"];
    posHouseNumber = eventDetailList["houseNumber"];
    prefecture.text = eventDetailList["prefecture"];
    city.text = eventDetailList["city"];
    houseNumber.text = eventDetailList["houseNumber"];

    phoneCon = TextEditingController(text: eventDetailList["phone"]);
    phoneNumber = eventDetailList["phone"];

    parts = eventDetailList["phone"].split('-');
    phoneFrCon = TextEditingController(text: parts[0]);
    phoneCenCon = TextEditingController(text: parts[1]);
    phoneBaCon = TextEditingController(text: parts[2]);
    firstPhone = parts[0];
    centralPhone = parts[1];
    backPhone = parts[2];

    emailCon = TextEditingController(text: eventDetailList["mail"]);
    email = eventDetailList["mail"];

    homepageCon = TextEditingController(text: eventDetailList["url"]);
    homepage = eventDetailList["url"];

    fee = eventDetailList["fee"];
    if (eventDetailList["fee"] == "無料") {
      feeController = TextEditingController(text: "");
      selectedPrace = "無料";
    } else {
      feeController = TextEditingController(text: eventDetailList["fee"]);
      selectedPrace = "有料";
    }

    capaController =
        TextEditingController(text: eventDetailList["Capacity"].toString());
    ...
    addCon = TextEditingController(text: eventDetailList["addMessage"]);
    additionalMessage = eventDetailList["addMessage"];

    cautionCon = TextEditingController(text: eventDetailList["notes"]);
    caution = eventDetailList["notes"];

    for (int i = 0; i < eventDetailList["tag"].length; i++) {
      tagname.add({"name": eventDetailList["tag"][i]["name"]});
    }

    for (int i = 0; i < eventDetailList["eventTimes"].length; i++) {
      dayTimeGetStr = eventDetailList["eventTimes"][i]["start_time"];
      dayTimeGetFin = eventDetailList["eventTimes"][i]["end_time"];
      dayTimeGet = "$dayTimeGetStr ～\n $dayTimeGetFin";
      eventTime.add({"start_time": dayTimeGet, "end_time": "22"});
      poseventTime.add({
        "start_time": eventDetailList["eventTimes"][i]["start_time"],
        "end_time": eventDetailList["eventTimes"][i]["end_time"]
      });
    }
  }
//------------------------------------------------------------------------

  Future userInfoUpdata(
    ChangeGeneralCorporation store,
    int id,
    ...
    try {
      final response = await put(url,
          ...
          body: json.encode(eventList["event"]));
                            ...
                            setState(() {
                              ...
                              if (shortageList.isEmpty) {
                                ...
                                if (value) {
                                  ...
                                  userInfoUpdata(store, eventDetailList["id"],
                                          dbPostList)
                                      .then((success) {
                                    //ここでローディング画面を表示
                                    if (success) {
                                      ...
                                      Navigator.push(
                                        ...
                                        MaterialPageRoute(
                                          builder: (context) => FinishScreen(
                                            appbarText: "イベント内容変更完了",
                                            appIcon: Icons.mail_outlined,
                                            finishText: "イベント内容を更新しました。",
                                            text:
                                                "イベント内容変更を完了しました。\nこの画面を閉じるには以下のボタンを押してください。",
                                            buttonText: "閉じる",
