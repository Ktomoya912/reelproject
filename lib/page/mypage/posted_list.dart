...
class _PostedListState extends State<PostedList> {
  ...
  Future getJobList(ChangeGeneralCorporation store) async {
    Uri url = Uri.parse(
        '${ChangeGeneralCorporation.apiUrl}/jobs/?${ChangeGeneralCorporation.sortId}&order=asc&offset=0&limit=50&${ChangeGeneralCorporation.typePosted}&user_id=${store.myID}&target=posted');
    final response = await http.get(url, headers: {
      'accept': 'application/json',
    });
    final data = utf8.decode(response.bodyBytes);
